...
public class Flocker extends Follower {
	...
	// passed percept list.
        for (int i = 0; i < ps.size(); i++) {
    		if (ps.get(i).getObjectCategory().name() == "OBSTACLE" ||
    				ps.get(i).getObjectCategory().name() == "PREDATOR") {
    			if (ps.get(i).getDistance() <= DEFAULT_CLEARANCE_DISTANCE) {
    				if (Math.abs(ps.get(i).getAngle()) <= flocking.cone) {
    					double weight = OBSTACLE_WEIGHT * (DEFAULT_CLEARANCE_DISTANCE / ps.get(i).getDistance());
    					if (ps.get(i).getAngle() <= 0) {
    						mf = new WeightedForce(weight, flocking.cone);
    					} else {
    						mf = new WeightedForce(weight, (flocking.cone * -1));
    					}
    				}
    			}
    		}
	...
	// based on the passed percept list  
        int counter = 0;
        for (int i = 0; i < ps.size(); i++) {
    		if (ps.get(i).getObjectCategory().name() == "BOID") {
    			if (ps.get(i).getDistance() <= DEFAULT_SEPARATION_DISTANCE) {
    				double weight = SEPARATION_WEIGHT * (DEFAULT_SEPARATION_DISTANCE / ps.get(i).getDistance());
    				if (counter == 0) {
    					sf = new WeightedForce(weight, ps.get(i).getAngle() + Math.PI);
    					counter++;
    				} else {
    					WeightedForce tempsf = new WeightedForce();
    					tempsf = new WeightedForce(weight, ps.get(i).getAngle() + Math.PI);
    					sf.addIn(tempsf);
    					counter++;
    				}
    			}
	...
	// based on the passed percept list
        ...
        for (int i = 0; i < ps.size(); i++) {
    		if (ps.get(i).getObjectCategory().name() == "BOID") {
    			if (ps.get(i).getDistance() <= DEFAULT_DETECTION_DISTANCE &&
    					ps.get(i).getDistance() >= DEFAULT_SEPARATION_DISTANCE) {
    				double weight = ALIGNMENT_WEIGHT;
    				if (counter == 0) {
    					af = new WeightedForce(weight, ps.get(i).getOrientation());
    					counter++;
    				} else {
    					WeightedForce tempsf = new WeightedForce();
    					tempsf = new WeightedForce(weight, ps.get(i).getOrientation());
    					af.addIn(tempsf);
    					counter++;
    				}
    			}
	...
	// based on the passed percept list  
        ...
        for (int i = 0; i < ps.size(); i++) {
    		if (ps.get(i).getObjectCategory().name() == "BOID") {
    			if (ps.get(i).getDistance() <= DEFAULT_DETECTION_DISTANCE) {
    				double tempX = ps.get(i).getDistance() * Math.cos(ps.get(i).getAngle());
    				double tempY = ps.get(i).getDistance() * Math.sin(ps.get(i).getAngle());
    				/*avgX += tempX;
    				avgY += tempY;
    				counter++;*/
    				/*if (tempX > maxX) {
    				...
    				double weight = CENTERING_WEIGHT * (ps.get(i).getDistance() / DEFAULT_DETECTION_DISTANCE);
    				if (counter == 0) {
    					cf = new WeightedForce(weight, ps.get(i).getAngle());
    					counter++;
    				} else {
    					WeightedForce tempsf = new WeightedForce();
    					tempsf = new WeightedForce(weight, ps.get(i).getAngle());
    					cf.addIn(tempsf);
    					counter++;
    				}
    			}
        ...
        /*avgX = avgX / counter;
        avgY = avgY / counter;*/
        //double weight = CENTERING_WEIGHT * ((Math.sqrt((avgX * avgX) + (avgY * avgY))) / DEFAULT_DETECTION_DISTANCE);
        //cf = new WeightedForce(weight, Math.atan(avgY/avgX));
        return cf;
    }
	...
	// a particular target in the passed percept list
    	for (int i = 0; i < ps.size(); i++) {
    		if (ps.get(i).getObjectCategory().name() == "LIGHT") {
    			double weight = LIGHT_WEIGHT;
    			ff = new WeightedForce(weight, ps.get(i).getAngle());
    		}
    	}
    	return ff;
    }
